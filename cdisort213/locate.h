...
DISPATCH_MACRO int locate_disort (double *xx, int n, double x);
DISPATCH_MACRO int flocate_disort (float *xx, int n, float x);

/* The function bodies (formerly locate.c) are header-inline so that they
 * can compile as __host__ __device__ for the CUDA build. */

DISPATCH_MACRO inline int locate_disort (double *xx, int n, double x)
...
DISPATCH_MACRO inline int flocate_disort (float *xx, int n, float x)
/* Given an array xx[1..n], and given a value x, returns a value j such that x is between xx[j] */
/* and xx[j+1]. xx must be monotonic, either increasing or decreasing. j=0 or j=n is returned   */
/* to indicate that x is out of range. A unit-offset array xx is assumed. To use locate_disort with    */
/* a zero-offset array, remember to subtract 1 from the address of xx, and also                 */
