...
void print_test(disort_state *ds_calc, disort_output *calc,
                disort_state *ds_good, disort_output *good) {
  register int iu, j, lu, numbad;
  // extern void
  //   c_errmsg();
  // extern double
  //  c_ratio();
  double flxmax, umax, fnoise, unoise, rat1, rat2, rat3, rat4,
      ratv[ds_good->nphi];

  flxmax = 0.0;
  for (lu = 0; lu < ds_good->ntau; lu++) {
    flxmax = MAX(MAX(MAX(flxmax, good->rad[lu].rfldir), good->rad[lu].rfldn),
                  good->rad[lu].flup);
  }
  ...
  for (lu = 1; lu <= ds_good->ntau; lu++) {
    fprintf(stdout, "%11.4f%15.4e%15.4e%15.4e%15.4e\n", UTAU(lu),
            calc->rad[lu - 1].rfldir, calc->rad[lu - 1].rfldn,
            calc->rad[lu - 1].flup, calc->rad[lu - 1].dfdt);

    fprintf(stdout, "%11.4f%15.4e%15.4e%15.4e%15.4e\n", UTAU(lu),
            good->rad[lu - 1].rfldir, good->rad[lu - 1].rfldn,
            good->rad[lu - 1].flup, good->rad[lu - 1].dfdt);
