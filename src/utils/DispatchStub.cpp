...
static CPUCapability compute_cpu_capability() {
  ...
  if (envar.has_value()) {
    ...
    int sve_vl = cpuinfo_get_max_arm_sve_length();  // Returns maximum SVE VL
                                                    // supported by your HW.
    ...
    if (envar == "sve256") {
      ...
      TORCH_WARN(
          "SVE256 capability not available on hardware. Falling back to "
          "DEFAULT");
    ...
    TORCH_WARN("ignoring invalid value for ATEN_CPU_CAPABILITY: ",
               envar.value());
  }

#if !defined(__powerpc__) && !defined(__s390x__) && \
    !defined(HAVE_SVE_CPU_DEFINITION)
...
#ifdef HAVE_ARM_BF16_CPU_DEFINITION
      if (cpuinfo_has_arm_bf16()) return CPUCapability::SVE256;
#endif
    }
#endif
    // Return the default CPU capability.
...
DispatchResult DispatchStubImpl::try_get_call_ptr(const DeviceType device_type,
                                                  void *DEFAULT
#ifdef HAVE_AVX512_CPU_DEFINITION
                                                  ,
                                                  void *AVX512
  ...
  constexpr auto supported_devices = c10::array_of<c10::DeviceType>(
      c10::DeviceType::CPU, c10::DeviceType::CUDA, c10::DeviceType::HIP,
      c10::DeviceType::MPS, c10::DeviceType::MTIA, c10::DeviceType::XPU,
      c10::DeviceType::HPU, c10::DeviceType::PrivateUse1);
  ...
  switch (device_type) {
    case DeviceType::CPU: {
      ...
      if (!fptr) {
        auto result = try_choose_cpu_impl(DEFAULT
#ifdef HAVE_AVX512_CPU_DEFINITION
                                          ,
                                          AVX512
#endif
    ...
    case DeviceType::CUDA:
      return cuda_dispatch_ptr != nullptr ? DispatchResult(cuda_dispatch_ptr)
                                          : ErrorType::MissingDeviceKernel;

    case DeviceType::HIP:
      return hip_dispatch_ptr != nullptr ? DispatchResult(hip_dispatch_ptr)
                                         : ErrorType::MissingDeviceKernel;

#if defined(USE_MPS)
    case DeviceType::MPS:
      return mps_dispatch_ptr != nullptr ? DispatchResult(mps_dispatch_ptr)
                                         : ErrorType::MissingDeviceKernel;
#endif
    case DeviceType::MTIA:
      return mtia_dispatch_ptr != nullptr ? DispatchResult(mtia_dispatch_ptr)
                                          : ErrorType::MissingDeviceKernel;

#if defined(USE_XPU)
    case DeviceType::XPU:
      return xpu_dispatch_ptr != nullptr ? DispatchResult(xpu_dispatch_ptr)
                                         : ErrorType::MissingDeviceKernel;
#endif

    case DeviceType::PrivateUse1:
      return privateuse1_dispatch_ptr != nullptr
                 ? DispatchResult(privateuse1_dispatch_ptr)
                 : ErrorType::MissingDeviceKernel;

    default:
      TORCH_INTERNAL_ASSERT(false, "An unexpected device type was provided ",
                            device_type);
      return ErrorType::DeviceNotSupported;
  }
}

void *DispatchStubImpl::get_call_ptr(const DeviceType device_type, void *DEFAULT
#ifdef HAVE_AVX512_CPU_DEFINITION
                                     ,
                                     void *AVX512
#endif
  ...
  auto result = try_get_call_ptr(device_type, DEFAULT
#ifdef HAVE_AVX512_CPU_DEFINITION
                                 ,
                                 AVX512
#endif
  ...
  if (std::holds_alternative<ErrorType>(result)) {
    ...
    switch (error) {
      case ErrorType::MissingDeviceKernel:
        TORCH_INTERNAL_ASSERT(false, "DispatchStub: missing kernel for ",
                              device_type);
        return nullptr;
      case ErrorType::DeviceNotSupported:
        TORCH_CHECK(false, "DispatchStub: unsupported device type",
                    device_type);
    }
  }
...
DispatchResult DispatchStubImpl::try_choose_cpu_impl(void *DEFAULT
#ifdef HAVE_AVX512_CPU_DEFINITION
                                                     ,
                                                     void *AVX512
#endif
  ...
  if (capability >= static_cast<int>(CPUCapability::AVX512)) {
    ...
    if (C10_UNLIKELY(!AVX512)) {
      ...
      return AVX2 != nullptr ? DispatchResult(AVX2)
                             : ErrorType::MissingDeviceKernel;
    } else {
  ...
  if (capability >= static_cast<int>(CPUCapability::VSX)) {
    return VSX != nullptr ? DispatchResult(VSX)
                          : ErrorType::MissingDeviceKernel;
  }
#endif
#ifdef HAVE_ZVECTOR_CPU_DEFINITION
  if (capability >= static_cast<int>(CPUCapability::ZVECTOR)) {
    return ZVECTOR != nullptr ? DispatchResult(ZVECTOR)
                              : ErrorType::MissingDeviceKernel;
  }
#endif
#ifdef HAVE_SVE256_CPU_DEFINITION
  if (capability >= static_cast<int>(CPUCapability::SVE256)) {
    if (C10_UNLIKELY(!SVE256)) {
      ...
      return DEFAULT != nullptr ? DispatchResult(DEFAULT)
                                : ErrorType::MissingDeviceKernel;
    } else {
